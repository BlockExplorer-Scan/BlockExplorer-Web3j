...
public class ThreadTaskTransaction implements Runnable, Serializable {
    ...
    public void run() {
        ...
        try {
            ...
            if (tx.getValueRaw().equals("0x0")) {
                logger.info("内部交易成功 -- 区块高度 : {} -- 内部交易hash ：{}  -- 内部交易from ：{} -- 内部交易to ：{}  -- GasUsed : {} -- start",
                        tx.getBlockNumber(), tx.getHash(), tx.getFrom(), tx.getTo(), jsonObjectTx.get("gasUsed"));
                if(tx.getTo() == null || "".equals(tx.getTo())){
                    logger.info("合约创建 -- 区块高度 : {} -- 内部交易hash ：{}  -- 内部交易from ：{} -- 内部交易to ：{}  -- GasUsed : {} -- start",
                            tx.getBlockNumber(), tx.getHash(), tx.getFrom(), tx.getTo(), jsonObjectTx.get("gasUsed"));
                    //验证是否是创建合约
                ...
                logger.info("内部交易成功 -- 区块高度 : {} -- 内部交易hash ：{}  -- 内部交易from ：{} -- 内部交易to ：{}  --  GasUsed : {} -- end",
                        tx.getBlockNumber(), tx.getHash(), tx.getFrom(), tx.getTo(), jsonObjectTx.get("gasUsed"));
            ...
            ex.printStackTrace();
            logger.error("   joon  ThreadTaskTransaction Exception :{}", ex.getMessage());
        }
     ...
     */
    private void traceTransactionHandel() {
        try {
            ...
            // 判断calls下面的value是否等于0x0 如果不等于则为 主币内部转账· 020-34626121   6230  3347
            getCallsByValueNeq0x0(callTracerMain);
            ...
            logger.error("   joon  ThreadTaskTransaction -- traceTransactionHandel" +
                    " -- 区块高度 : {} -- 区块hash : {} -- 交易hash : {} - Exception :{}",
                    tx.getBlockNumber(),tx.getBlockHash(),tx.getHash(),ex.getMessage());
            ex.printStackTrace();
//            traceTransactionHandel();
...
//            traceTransactionHandel();
     ...
     * @return
     */
    private void getCallsByValueNeq0x0(CallTracerMain callTracerMain) throws Exception {

        List<CallTracer> callTracers = callTracerMain.getCalls();

        for (int i = 0; i < callTracers.size(); i++) {
            if (!callTracers.get(i).getValue().equals("0x0")) {
